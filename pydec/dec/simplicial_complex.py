...
class simplicial_complex(list):
    ...
    def compute_bary_circumcenters(self,dim):
        ...
        """
        data = self[dim]            
        data.bary_circumcenter = torch.zeros((data.num_simplices,
                                        dim + 1))
    ...
    def compute_circumcenters(self,dim):
        ...
        """
        data = self[dim]            
        data.circumcenter = torch.zeros((data.num_simplices,
    ...
    def compute_primal_volume(self,dim):
        ...
        """
        data = self[dim]
        data.primal_volume = torch.zeros((data.num_simplices,))
    ...
    def compute_dual_volume(self):
        ...
        """
        for dim,data in enumerate(self):
            data.dual_volume = torch.zeros((data.num_simplices,))

        temp_centers = torch.zeros((self.complex_dimension() + 1,
                                  self.embedding_dimension()))
        temp_bcenters = [torch.zeros(dim) for dim in
    ...
    def __compute_dual_volume(self, s, parent, pts, bpts, signs, dim):    
        ...
        if parent is not None:
            ...
            signs[dim] = torch.copysign(torch.tensor([1]), bpts[-1][ov_index])
